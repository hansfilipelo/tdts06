...
if __name__ == "__main__":
    ...
    programOption = int(sys.argv[2].replace("option=",""))
    ...
    for line in sys.stdin:
        ...
        if priority <= minPriority: # If priority of this file is lower than the ones we cache, dont't cache
            misses += 1
            filesNeverInCache += 1
        ...
        if fileName in cache:
            ...
            if cacheSize+size < cacheMaxSize:
                cache[fileName] = {"hits" : 0, "size" : size, "priority" : priority}
                cacheSize += size
            else:
                while cacheSize+size > cacheMaxSize:
                    removedElement = cache.pop(next(iter(cache.keys())))
                ...
                cache[fileName] = {"hits" : 0, "size" : size}
                cacheSize += size
